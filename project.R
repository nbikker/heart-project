...
# This .r analyzes a dataset that has 13 features that can be used to predict heart failure
...
# ejection fraction, age and obviously time as being the best predictors of heart failure by death
...
# skim data using skimr to identify types of variables 
skim(df)
...
logistic_regression <- glm(formula = DEATH_EVENT ~ ., data = df[train, ]
                           , family = "binomial")
summary(logistic_regression)
# from the summary statistics, age, ejection fraction, serum creatinine, 
# and time are significant predictors
# these are significant because p-value is less than 0.05

#predict target variable 

prediction <- predict(logistic_regression)
